...
struct SCollisionHit
{
	...
	//! Triangle selector which contained the colliding triangle (useful when having
	//! MetaTriangleSelector)
	ITriangleSelector *TriangleSelector;

	//! Node which contained the triangle (is 0 when selector doesn't have that
	//! information)
	ISceneNode *Node;

	//! Meshbuffer which contained the triangle (is 0 when the selector doesn't have
	//! that information, only works when selectors are created per meshbuffer)
	const IMeshBuffer *MeshBuffer;

	//! Index of selected material of the triangle in the SceneNode. Usually only
	//! valid when MeshBuffer is also set, otherwise always 0
...
{
public:
	//! Finds the nearest collision point of a line and lots of triangles, if there is
	//! one.
	/** \param hitResult: Contains collision result when there was a collision
	detected. \param ray: Line with which collisions are tested. \param selector:
	TriangleSelector to be used for the collision check. \return true if a collision
	was detected and false if not.	*/
	virtual bool getCollisionPoint(SCollisionHit &hitResult,
			const core::line3d<f32> &ray, ITriangleSelector *selector) = 0;
	...
	virtual core::vector3df getCollisionResultPosition(ITriangleSelector *selector,
			const core::vector3df &ellipsoidPosition,
			const core::vector3df &ellipsoidRadius,
			const core::vector3df &ellipsoidDirectionAndSpeed,
			core::triangle3df &triout, core::vector3df &hitPosition,
			bool &outFalling, ISceneNode *&outNode,
			f32 slidingSpeed = 0.0005f,
			const core::vector3df &gravityDirectionAndSpeed = core::vector3df(
					0.0f, 0.0f, 0.0f)) = 0;
	...
	virtual core::line3d<f32> getRayFromScreenCoordinates(
			...
			const ICameraSceneNode *camera = 0) = 0;
	...
	virtual core::position2d<s32> getScreenCoordinatesFrom3DPosition(
			const core::vector3df &pos, const ICameraSceneNode *camera = 0,
			bool useViewPort = false) = 0;

	//! Gets the scene node, which is currently visible under the given screen
	//! coordinates, viewed from the currently active camera.
	/** The collision tests are done using a bounding box for each
	scene node. You can limit the recursive search so just all children of the
	specified root are tested. \param pos: Position in pixel screen coordinates, under
	which the returned scene node will be. \param idBitMask: Only scene nodes with an
	id with bits set like in this mask will be tested. If the BitMask is 0, this
	feature is disabled.
	...
	\param root If different from 0, the search is limited to the children of this
	node. \return Visible scene node under screen coordinates with matching bits in
	its id. If there is no scene node under this position, 0 is returned. */
	virtual ISceneNode *getSceneNodeFromScreenCoordinatesBB(
			const core::position2d<s32> &pos, s32 idBitMask = 0,
			bool bNoDebugObjects = false, ISceneNode *root = 0) = 0;

	//! Returns the nearest scene node which collides with a 3d ray and whose id
	//! matches a bitmask.
	/** The collision tests are done using a bounding box for each
	...
	\param root If different from 0, the search is limited to the children of this
	node. \return Scene node nearest to ray.start, which collides with the ray and
	matches the idBitMask, if the mask is not null. If no scene node is found, 0 is
	returned. */
	virtual ISceneNode *getSceneNodeFromRayBB(const core::line3d<f32> &ray,
			s32 idBitMask = 0, bool bNoDebugObjects = false,
			ISceneNode *root = 0) = 0;

	//! Get the scene node, which the given camera is looking at and whose id matches
	//! the bitmask.
	...
	//! Perform a ray/box and ray/triangle collision check on a hierarchy of scene
	//! nodes.
	...
	bitmask, and which have a triangle selector are considered as candidates for being
	hit. You do not have to build a meta triangle selector; the individual triangle
	selectors of each candidate scene node are used automatically.
	...
	virtual ISceneNode *getSceneNodeAndCollisionPointFromRay(SCollisionHit &hitResult,
			const core::line3df &ray, s32 idBitMask = 0,
			ISceneNode *collisionRootNode = 0,
			bool noDebugObjects = false) = 0;

	//! Perform a ray/box and ray/triangle collision check on a hierarchy of scene
	//! nodes.
	/** Works same as other getSceneNodeAndCollisionPointFromRay but returns less
	information. (was written before the other getSceneNodeAndCollisionPointFromRay
	implementation). \param ray: Line with which collisions are tested. \param
	outCollisionPoint: If a collision is detected, this will contain the position of
	the nearest collision. \param outTriangle: If a collision is detected, this will
	contain the triangle with which the ray collided. \param idBitMask: Only scene
	nodes with an id which matches at least one of the bits contained in this mask
	will be tested. However, if this parameter is 0, then all nodes are checked.
	\param collisionRootNode: the scene node at which to begin checking. Only this
	node and its children will be checked. If you want to check the entire scene,
	pass 0, and the root scene node will be used (this is the default).
	\param noDebugObjects: when true, debug objects are not considered viable targets.
	...
	virtual ISceneNode *getSceneNodeAndCollisionPointFromRay(const core::line3df &ray,
			...
			ISceneNode *collisionRootNode = 0, bool noDebugObjects = false)
	{
		SCollisionHit hitResult;
		ISceneNode *node = getSceneNodeAndCollisionPointFromRay(hitResult, ray,
				idBitMask, collisionRootNode, noDebugObjects);
		if (node) {
			outCollisionPoint = hitResult.Intersection;
			outTriangle = hitResult.Triangle;
		}
